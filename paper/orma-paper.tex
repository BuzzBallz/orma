...
\begin{abstract}
\noindent
This is the technical paper for Orma. It states the arithmetic the protocol runs on, so that
a reader can check it rather than take it on trust, and without reading JavaScript. Every formula is implemented in the repository, and every
...
% The title page carries the title and the abstract and nothing else. A jury reading this
% on a laptop meets the claim first, on its own, rather than half a claim and the opening
% of section 1 competing for the same screen.
\clearpage
...
% This paragraph says "the first row below" and must not be separated from the table it
% points at. Clearing the title page shifted the pagination and stranded it once already.
\pagebreak
