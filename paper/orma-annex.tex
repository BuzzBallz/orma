...
value the protocol admits.

\paragraph{Rate scale.} $c_{\min}$ and $c_{\mathrm{liq}}$ are in units of $10^{-5}$. A
stated \texttt{10000} means $0.10$. It does not mean $10000$ and it does not mean $100$.
...
\noindent
The Kestrel broker owner is \texttt{rD26gw5goDqU784caePMoA82givcJBjStk}.
...
\midrule
\multicolumn{3}{l}{\texttt{PreviousFields} emitted for the Vault node: \texttt{\{\}}} \\
\bottomrule
\end{tabular}
\end{center}
...
\]
the second truncated to the $6$ decimal places the share token is scaled to. The divergence
...
Only the second term depends on $\sigma$, and it enters with a minus sign. Minimising total
cover consumed is therefore the same problem as maximising
...
computations agree: the iterative simulation that mirrors the ledger step by step, the
closed form, and the ledger itself.

Identical losses were realised in both rows; the only difference is the order of two
transactions eight seconds apart. The $0.20$ XRP, $40\%$ of what the first-loss capital
would otherwise have absorbed, was borne by the vault's investors, and the party who chose
the order is the party it spared.
...
exactly the non-performing book net of recoverable cover. Being fully lent is not a distress
signal, and this construction is what stops it reading as one.
\end{description}

The bands are stated in full so they can be disputed. Each row is a floor: the first
threshold the value meets or exceeds gives the grade, and a value below every threshold
grades \texttt{D}.
...
Recognition lag is banded on seconds rather than a ratio: $0$ is AAA, under $60$ is A, under
$300$ is BBB, under $900$ is BB, beyond that CCC.
...
condition & $\Delta$ \\
\midrule
realised capital destruction $\rho \geq 75\%$ & $-14$ \\
$\rho \geq 50\%$ & $-12$ \\
$\rho \geq 25\%$ & $-8$ \\
$\rho \geq 10\%$ & $-5$ \\
$\rho \geq 2\%$ & $-3$ \\
...
recognition lag $> 0$ s and $\leq 300$ s & $-1$ \\
largest exposure $\geq 75\%$ of the book & $-1$ \\
liquidatable cover $< 5\%$ of the exposure it must absorb & $-1$ \\
liquidity band at B or worse while in the Redemption phase & $-2$ \\
\bottomrule
\end{tabular}
\end{center}
...
\addlinespace
$V_{\mathrm{lendable}}$ at $h = 0.20$, correctly applied & $0.643137$ \\
...
whole record. A facility with a clean score may simply be younger than the window.
...
full subscription cash-flow history rather than $S$ alone; Orma declines to compute $\rho$
there rather than compute it wrongly.
...
so $1960.78$ is published as $1961$; unit values are truncated to the share token's $6$
decimal places, and oracle values to the dimension's scale, $6$ decimals for NAV and $4$ for
the other five. No figure here depends on a digit below those scales. The thresholds and
penalties in Section~5 are not derived from anything: they are a stated, ordinal,
reproducible judgement, published in full precisely so a reader who disagrees can say where
and by how much. The two results that are not judgements are Proposition~\ref{prop:empty}
and Theorem~\ref{thm:order}, which follow from the protocol rules and hold whatever anyone
thinks of the bands.
