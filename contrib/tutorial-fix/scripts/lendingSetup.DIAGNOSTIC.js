...
// --- DIAGNOSTIC INSTRUMENTATION (not part of the upstream file) ---
console.log('')
console.log('VaultCreate   result:', vaultCreateResponse.result.meta.TransactionResult, 'hash:', vaultCreateResponse.result.hash)
console.log('LoanBrokerSet result:', loanBrokerSetResponse.result.meta.TransactionResult, 'hash:', loanBrokerSetResponse.result.hash)
const _v = await client.request({ command: 'ledger_entry', index: vaultID, ledger_index: 'validated' })
console.log('Vault on ledger:', JSON.stringify({ VaultKind: _v.result.node.VaultKind, SubscriptionDate: _v.result.node.SubscriptionDate, RedemptionDate: _v.result.node.RedemptionDate, LedgerEntryType: _v.result.node.LedgerEntryType }))
console.log('LoanBrokerSet AffectedNodes types:', JSON.stringify(loanBrokerSetResponse.result.meta.AffectedNodes.map(n => { const k = Object.keys(n)[0]; return k + ':' + n[k].LedgerEntryType })))
